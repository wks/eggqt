...
/**
 * [wks] RGB是Windows API里的宏。我们不引入Windows头文件，而是自己定义RGB宏。
 */
#define RGB(r, g, b) ((unsigned long)(r) | ((unsigned long)(g) << 8) | ((unsigned long)(b) << 16))
 ...
 * 图操作都将使用此设置。
 * [wks] 时代变了。eggqt里缺省宽度为0.05cm。在这个1080p成为标配，2160p逐步
 * 普及的年代，一个像素肉眼基本上看不见。Qt对于HiDPI的做法是对程序员暴露“平台
 * 无关像素”，在不同的显示器上，会按照不同的倍率缩放成实际的像素。画图一般来说都
 * 以浮点数表达大小，以保证在不同的平台上以最佳的方式渲染。
